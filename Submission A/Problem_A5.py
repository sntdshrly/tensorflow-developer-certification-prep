...
def solution_A5():
    ...
    # YOUR CODE
    model.compile(loss='mae', optimizer=tf.keras.optimizers.SGD(lr=1e-6, momentum=0.9), metrics=['mae'])
    callback = MyCallback()
    model.fit(train_set, batch_size=256, epochs=100, callbacks=[callback])
    return model
